        ...
        <div class="line">
          <div class="margin">
            ...
            <div class="s-12 m-12 l-6">
              ...
              <form name="contactForm" class="customform" method="post" action="{{ route('buku.update', $books->id)}}">
                {{ csrf_field() }} {{method_field('put')}}
                <div class="line">
                    <div class="margin">
                      <div class="s-12">
                        <input name="judul" value="{{$books->judul}}" class="judul" placeholder="Judul Buku" title="Judul" type="text" />
                        <p class="title-error form-error">Masukkan Judul Buku</p>
                      </div>
                    </div>
                  </div>
                  <div class="s-12"> 
                    <input name="pengarang" value="{{$books->pengarang}}" class="pengarang" placeholder="Pengarang" title="Pengarang" type="text" />
                    <p class="title-error form-error">Masukkan Pengarang</p>
                  </div>
                  <div class="s-12"> 
                      <input name="penerbit" value="{{$books->penerbit}}" class="penerbit" placeholder="penerbit" title="penerbit" type="text" />
                    ...
                    <div class="s-12"> 
                        ...
                        <option value="{{ $value }}" {{ $books->tahun == $value ? 'selected' : ''}}>{{$value}}</option>
                          @endforeach
                        </select>
                      </div>
